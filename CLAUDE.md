...
- `Dockerfile` — Debian trixie-slim based image with dev tools, Claude Code (native installer), mise (runtime version manager), and Ralph. Runs as non-root user `dev` with UID/GID passed as build args (defaults to 1000). Versions of Claude Code, mise, and Ralph are pinned via `ARG` (`CLAUDE_CODE_VERSION`, `MISE_VERSION`, `RALPH_REF`); override with `--build-arg` at build time.
...
- `nosusp.py` — Python PTY proxy installed to `/usr/local/bin/nosusp` in the image. Wraps the claude invocation and strips `0x1A` (Ctrl+Z) out of the stdin byte stream so claude can't self-suspend.
...
- A named volume `claudepod-home` persists the container user's home directory (Claude config, shell history, etc.) across sessions. The volume carries an `image-id` label; if it doesn't match the current image's ID, the volume is removed (`podman volume rm --force`) and recreated so home dotfiles stay in sync with the image. A zombie container from a prior crashed run can block the removal — that case degrades to a warning and reuses the old volume rather than aborting the launch.
...
- **Ctrl+Z is filtered at the PTY layer.** Claude Code installs its own raw-mode SIGTSTP handler, so neither `stty susp undef` nor inherited `SIG_IGN` stops it from self-suspending (and the earlier fg-loop supervisor failed because `bash -c` non-interactive shells don't properly hand the terminal foreground pgid to a background child). Instead, `claudepod` prefixes the claude invocation with `nosusp` — a ~100-line Python stdlib PTY proxy (shipped at `/usr/local/bin/nosusp`, source: `nosusp.py`) that forks claude into a pseudo-terminal, puts the real tty in raw mode, and filters `0x1A` (Ctrl+Z) out of the stdin byte stream before forwarding to claude. Claude never sees the keystroke, so it can't raise SIGTSTP on itself. The proxy also forwards SIGWINCH-driven resizes and restores terminal state on exit. `-s/--shell` mode bypasses `nosusp` since a shell legitimately wants job control.
- The firewall requires `NET_ADMIN` and `NET_RAW` capabilities and runs via a sudoers rule limited to the firewall script only.
- The firewall resolves domain allowlist entries to IPs at container start using `dig`, fetches GitHub's IP ranges from `api.github.com/meta`, and fetches Bitbucket's IP ranges from `ip-ranges.atlassian.com` for SSH restrictions. The allowlist covers Anthropic, GitHub/Bitbucket, package registries (npm/PyPI/crates.io/Go proxy/Hex.pm), other AI API providers (ElevenLabs, Deepgram, Google Generative Language), and a broad set of language/framework/ML documentation sites (MDN, Stack Overflow, LangChain, Qdrant, MCP, etc.).
...
The launcher (`claudepod`) assembles the container's `bash -c` command using positional arguments to avoid shell-escaping bugs. It builds an `ENTRYPOINT_SCRIPT` string that references `$1`, `$2`, … and passes the actual values (mise system-prompt hint, `-p` prompt text, passthrough args after `--`) as positional arguments to `bash -c`. When editing the launcher, preserve this pattern — inlining user-supplied strings directly into the script string will break on quotes, newlines, or shell metacharacters in prompts.
